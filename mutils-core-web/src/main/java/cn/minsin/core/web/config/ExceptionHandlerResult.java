...
public interface ExceptionHandlerResult<T> {
    ...
    @ResponseBody
    @ResponseStatus(HttpStatus.OK)
    default T exception(Exception e, HttpServletResponse response) {
        return this.createResultWithException(e, "未知异常");
    }

    default T createResultWithException(Exception e, String message) {
        return this.createResultWithException(e, message, false);
    }

    T createResultWithException(Exception e, String message, boolean isError);
