...
public interface FunctionalInterfaceUtil {
     ...
     * @param collector 收集器
     */
    static <R, S, A, RR> RR map(Collection<S> source, Function<S, R> function, Collector<R, A, RR> collector) {
     ...
     * @param collector 收集器
     */
    static <R, S, A, RR> RR map(Collection<S> source, Predicate<S> filter, Function<S, R> function, Collector<R, A, RR> collector) {
        return source.stream().filter(filter).map(function).collect(collector);
    }
     ...
     * 如果不为空则执行函数
     */
    static <P, R> R ifNotNullExecute(P param1, Function<P, R> consumer) {
        return ifNotNullExecuteOrDefault(param1, null, consumer);
    }
     ...
     */
    static <T> boolean containsAny(Iterable<T> data, Predicate<T> predicate) {
        for (T datum : data) {
